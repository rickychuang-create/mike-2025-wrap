...
// StatusBar logic (Preserved from original, though often hidden in modern mobile web apps)
...
function StatusBarText({ text, isDark = false }: StatusBarTextProps) {
  ...
  return (
    <div className="absolute h-[47px] left-0 overflow-clip top-0 w-full pointer-events-none z-50 hidden lg:block">
...
// This wrapper ensures the content takes 100svh on mobile (avoiding URL bar issues)
// and fixed dimensions on desktop.
function ResponsiveScreenWrapper({ 
  children, 
  background, 
  className = "",
  dataName,
  ...
  return (
    <div 
      ref={contentRef}
      className={`relative w-full h-[100svh] lg:h-[844px] lg:w-[390px] overflow-hidden flex flex-col ${className}`} 
      data-name={dataName}
    >
      ...
      {background && (
        <img 
          alt="" 
          className="absolute inset-0 max-w-none object-center object-cover pointer-events-none size-full z-0" 
          src={background} 
        />
      )}
      {/* Content Layer - Z-index ensures it sits above background */}
...
function Screen1({
  ...
  return (
    <ResponsiveScreenWrapper background={imgLoginBackground} className="bg-[#5B16D6]" dataName="Login">
      {/* Top Bar for Language - Fixed padding for safe area */}
      <div className="w-full flex justify-end px-8 pt-12 lg:pt-8 z-20">
      ...
      {/* Main Content - Centered vertically via flexbox */}
      <div className="flex-1 flex flex-col justify-center px-12 w-full pb-8">
...
function Screen2({ userData }: { userData: UserData | null }) {
  ...
  return (
    <ResponsiveScreenWrapper background={imgLoginBackground} className="bg-[#5B16D6]" dataName="Welcome">
      {/* Original decorative layers - kept absolute but with full width/height */}
      ...
      <div className="flex-1 flex flex-col items-center justify-between py-16 px-10 relative z-10">
        <div className="flex-1 flex flex-col items-center justify-center w-full">
            <p className="font-['Inter','Noto_Sans_SC'] font-semibold text-white text-[18px] leading-[1.3] text-center mb-6 uppercase tracking-wider">
            {language === 'zh' ? '与麦克同行的日子' : 'DAYS WITH MIKE'}
            </p>
            
            {/* Massive number - Scalable text for responsiveness */}
            <div className="w-full text-center">
                <h1 className="font-['Space_Grotesk','Noto_Sans_SC'] font-bold text-white text-[clamp(140px,20vw,180px)] leading-[0.85] tracking-[-0.04em] text-center">
                {days_with_mike}
                </h1>
            </div>
        </div>
...
function Screen3({ userData }: { userData: UserData | null }) {
  ...
  return (
    <ResponsiveScreenWrapper background={imgUserTypeBackground} className="bg-gradient-to-b from-[#7B3FE4] via-[#5B16D6] to-[#4A0FB8]" dataName="Days Logged">
      {/* Decor layers */}
      ...
      {/* Flex Layout */}
      <div className="flex-1 flex flex-col justify-between px-8 py-12 pt-20 relative z-10">
        ...
        {/* Main Stat - Centered in available space */}
        ...
        {/* Bottom Section: Secondary Stats + Card */}
...
function Screen4({ userData }: { userData: UserData | null }) {
  ...
  return (
    <ResponsiveScreenWrapper background={imgMikeTradeBackground} className="bg-[#5B16D6]" dataName="Top Functions">
      {/* Decorative Overlays */}
      ...
      <div className="flex-1 flex flex-col px-8 py-10 pt-20 relative z-10">
        <div className="mb-6">
        ...
        <div className={`flex-1 flex flex-col ${user_type === 1 ? 'justify-center gap-5' : 'justify-center gap-3'}`}>
            {features.map((feature, index) => {
            const { chinese, english } = splitFeatureName(feature.name);
            
            // Logic regarding height/size (preserved from original logic, just translated to tailwind classes if possible)
            // Using style for height to ensure exact logic match if classes aren't precise enough, but tailwind is usually fine.
            
            const getBarHeight = () => {
                if (user_type === 1) {
                switch(index) {
                    case 0: return 'h-[150px]'; 
                    case 1: return 'h-[110px]'; 
                    case 2: return 'h-[80px]'; 
                    default: return 'h-[80px]';
                }
                }
                switch(index) {
                case 0: return 'h-[84px]'; 
                case 1: return 'h-[72px]'; 
                case 2: return 'h-[64px]'; 
                case 3: return 'h-[56px]'; 
                case 4: return 'h-[48px]'; 
            ...
            const getRankNumberSize = () => {
                if (user_type === 1) {
                switch(index) {
                    case 0: return 'text-[80px]'; 
                    case 1: return 'text-[64px]'; 
                    case 2: return 'text-[48px]'; 
                    default: return 'text-[48px]';
                }
                }
                switch(index) {
                case 0: return 'text-[56px]'; 
                case 1: return 'text-[48px]'; 
                case 2: return 'text-[42px]'; 
                case 3: return 'text-[36px]'; 
                case 4: return 'text-[32px]'; 
            ...
            const getChineseFontSize = () => {
                if (user_type === 1) {
                switch(index) {
                    case 0: return 'text-[30px]'; 
                    case 1: return 'text-[24px]'; 
                    case 2: return 'text-[18px]'; 
                    default: return 'text-[18px]';
                }
                }
                switch(index) {
                case 0: return 'text-[20px]'; 
                case 1: return 'text-[18px]'; 
                case 2: return 'text-[17px]'; 
                case 3: return 'text-[16px]'; 
                case 4: return 'text-[15px]'; 
            ...
            const getEnglishFontSize = () => {
                ...
                switch(index) {
                    ...
                    case 2: return 'text-[12.5px]';
                    case 3: return 'text-[12px]';
                    case 4: return 'text-[11px]';
            ...
            const gap = user_type === 1 ? 'gap-2' : 'gap-4';
            const numberContainerWidth = user_type === 1 ? 'w-16' : 'w-14';
            const numberAlignment = user_type === 1 ? 'justify-start' : 'justify-center';
            const displayRank = user_type === 1 ? String(index + 1) : feature.rank;
            
            return (
                <div key={index} className={`relative flex items-center ${gap}`}>
                <div className={`flex-shrink-0 ${numberContainerWidth} flex items-center ${numberAlignment}`}>
                ...
                <div className="flex-1">
                    <div 
                    className={`${getBarHeight()} bg-white/15 backdrop-blur-xl ${borderRadius} flex items-center justify-between px-6 shadow-lg shadow-black/10 border border-white/40 relative overflow-hidden`}
                    >
        ...
        {/* VIP Features Summary - Pinned at bottom of container */}
        <div className="min-h-[80px] flex items-end">
            {hasVIPFeatures && (
            <div className="w-full bg-white/10 backdrop-blur-lg rounded-2xl p-5 border border-white/20 mt-4">
...
function Screen5({ userData }: { userData: UserData | null }) {
  const { language } = useLanguage();
  return (
    <ResponsiveScreenWrapper background={imgLoginBackground} className="bg-[#5B16D6]" dataName="Product Evolution">
      <img alt="" className="absolute inset-0 max-w-none object-center object-cover pointer-events-none size-full opacity-35" src={imgEvolutionBackground} />
      <div className="absolute inset-0 bg-gradient-to-br from-[#5B16D6]/25 via-transparent to-[#5B16D6]/15 mix-blend-overlay" />
      <div className="absolute inset-0 bg-gradient-to-t from-black/15 via-transparent to-black/10" />
      
      <div className="flex-1 flex flex-col px-8 py-10 pt-16 relative z-10">
        <div className="mb-6">
        ...
        {/* Timeline - Flex column with even distribution */}
        <div className="flex-1 flex flex-col justify-center relative pl-4 pb-4">
            ...
            <div className="flex flex-col gap-6 justify-center h-full">
                {[
                    ...
                    { month: 'AUG', img: imgMikeTrade, title: language === 'zh' ? '投资日志' : 'Investment Journal', desc: language === 'zh' ? '记录决策、回顾逻辑、培养更好的习惯' : 'Record decisions, review logic, and build better habits.' },
                    { month: 'NOV', img: imgLiveRoom, title: language === 'zh' ? '首页&语音聊天室' : 'Home Page &\nLive Room', desc: language === 'zh' ? '更快地探索 —— 以及全新的实时学习方式' : 'Explore faster — plus a new way to learn together in real time.' }
                ].map((item, index) => (
                    <div key={index} className="relative pl-16">
                        <div className="absolute left-0 top-0">
                            <div className={`w-12 h-12 rounded-full ${index === 2 ? 'bg-white/30 border-white/60 shadow-lg shadow-white/20' : 'bg-white/15 border-white/30'} backdrop-blur-lg border-2 flex items-center justify-center`}>
                        ...
                        <div className="bg-white/12 backdrop-blur-2xl rounded-2xl overflow-hidden border border-white/25 shadow-xl shadow-black/10">
                            <div className={`relative ${index === 2 ? 'h-[160px]' : 'h-[115px]'} overflow-hidden`}>
                                <img alt="" className="absolute inset-0 w-full h-full object-cover opacity-65" src={item.img} style={{ transform: 'scale(1.15)' }} />
                                <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/30 to-transparent" />
                                <div className="absolute inset-0 bg-gradient-to-t from-[#5B16D6]/40 via-[#5B16D6]/20 to-transparent" />
                                
                                <div className={`absolute bottom-3.5 left-4 right-4`}>
                                    <h3 className={`font-['Space_Grotesk','Noto_Sans_SC'] font-bold text-white ${index === 2 ? 'text-[28px]' : 'text-[24px]'} leading-[1.1] tracking-[-0.02em] mb-1.5`}>
                                        {item.title}
                                    </h3>
...
function Screen5a({ userData }: { userData: UserData | null }) {
  const { language } = useLanguage();
  return (
    <ResponsiveScreenWrapper background={imgMikeTradeBackground} className="bg-[#5B16D6]" dataName="Portfolio Performance">
      <div className="absolute inset-0 bg-gradient-to-br from-[#7B3FE4]/50 via-[#5B16D6]/40 to-[#4A0FB8]/50 mix-blend-color" />
      <div className="absolute inset-0 bg-[#5B16D6]/30 mix-blend-multiply" />
      <div className="absolute inset-0 bg-gradient-radial from-transparent via-transparent to-black/20" />
      <div className="absolute inset-0 bg-gradient-to-b from-black/15 via-transparent to-transparent" />
...
function Screen8({ userData }: { userData: UserData | null }) {
  ...
  // Icon component - ORIGINAL logic restored
  const getTypeIcon = () => {
    return (
      <svg className="absolute top-[140px] left-1/2 -translate-x-1/2 w-[280px] h-[280px] opacity-20 pointer-events-none" viewBox="0 0 280 280" fill="none">
  ...
  return (
    // Note: screenRef is passed to the wrapper so toPng captures the whole thing
    <ResponsiveScreenWrapper 
      contentRef={screenRef} 
      background={imgUserTypeBackground} 
      className="bg-gradient-to-b from-[#7B3FE4] via-[#5B16D6] to-[#4A0FB8]" 
      dataName="User Type"
    >
      <div className="absolute inset-0 bg-gradient-to-br from-[#5B16D6]/30 via-transparent to-[#5B16D6]/20 mix-blend-overlay" />
      <div className="absolute inset-0 bg-gradient-to-t from-black/20 via-transparent to-black/10" />
      
      {/* Icon - Absolute position but centered */}
      {getTypeIcon()}

      {/* Content Flex */}
      <div className="flex-1 flex flex-col items-center justify-between px-8 py-10 pt-20 relative z-10">
        ...
        <div className="text-center w-full">
            ...
            <p className="font-['Inter','Noto_Sans_SC'] font-normal text-white/90 text-[16px] leading-[1.5] text-center mt-4">
            {language === 'zh' ? '分析你的活动后，你是一位…' : "After analyzing your activity, you're a…"}
            </p>
        </div>
        
        {/* Main Title - Flexibly centered */}
        ...
        {/* Action Buttons */}
        <div className="w-full space-y-3" data-exclude-from-capture="true">
            <button 
...
function Screen9({ userData }: { userData: UserData | null }) {
  ...
  return (
    <ResponsiveScreenWrapper background={imgLoginBackground} className="bg-[#5B16D6]" dataName="Premium Reminder">
...
export default function MikeWrap() {
  ...
  ].filter(Boolean); // Filter out nulls (Screen9 if not VIP)
  ...
  return (
    <LanguageContext.Provider value={{ language, setLanguage }}>
      ...
      <div className="hidden lg:flex gap-8 p-8 h-full overflow-x-auto items-center">
        {screens.map((screen, index) => (
          <div key={index} className="flex-shrink-0 shadow-2xl rounded-3xl overflow-hidden border-[8px] border-black/80 h-[844px] w-[390px] ring-1 ring-white/10">
            {screen}
          </div>
        ))}
      </div>
      ...
      <div className="lg:hidden fixed inset-0 w-full h-[100svh] bg-[#5B16D6]">
        {/* Active Screen */}
        ...
        <div className="absolute top-[max(10px,env(safe-area-inset-top))] left-4 right-4 flex gap-1 z-50 pointer-events-none">
          {screens.map((_, index) => (
            <div 
              key={index}
        ...
        <div className="absolute inset-0 flex z-40">
          ...
          <div 
            ...
            onClick={(e) => {
              e.stopPropagation();
              if (currentScreen < screens.length - 1) setCurrentScreen(prev => prev + 1);
            }} 
          />
        </div>
      </div>
